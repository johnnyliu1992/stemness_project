...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 16 15:53:52 2022
...
celllines_17p_neutral=[i for i in list(cell_info['cell_id']) if i in cell_17p_neutral]
###### NO 'YAPC'
#celllines_17p_loss.remove('YAPC')

#check all instances for cell_17p_neutral
inst_info_17p_neutral=inst_info.loc[inst_info['cell_id'].isin(celllines_17p_neutral)]

#get all compound information
compound_count=inst_info_17p_neutral['pert_iname'].value_counts()
compound_list=list(set(inst_info_17p_neutral['pert_iname']))

#calculate before treatment stemness for all cell lines
#consider 'DMSO','UnTrt','EMPTY_VECTOR' as un_tre
un_tre_mean={}
for curr_cell in celllines_17p_neutral:
    curr_data=inst_info_17p_neutral.loc[inst_info_17p_neutral['pert_iname'].isin(['DMSO','UnTrt','EMPTY_VECTOR'])]
...
for curr_comp in compound_list:
    ...
    curr_inst=inst_info_17p_neutral.loc[inst_info_17p_neutral['pert_iname']==curr_comp]
    #loop cell lines
...
cell_line_counts_comp_final=summary_comp_lg5['comp'].value_counts()
#find compound insts have more than 5 cell line treated
comp_final=summary_comp_lg5.loc[summary_comp_lg5['comp'].isin(list(cell_line_counts_comp_final.loc[cell_line_counts_comp_final>=5].index))]
...
comp_final.to_csv('comp_summary_final_neutral.csv', index=False, header=True, sep='\t')
